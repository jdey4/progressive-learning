...
model_file_top = ['dnn0','fixed_uf10', 'model_zoo','Prog_NN', 'LMC', 'DF_CNN']
model_file_bottom = ['uf10', 'EWC', 'offline', 'exact', 'LwF', 'OEWC', 'si', 'er', 'agem', 'tag', 'None']
...
te_5000 = {'SynN':np.zeros(10,dtype=float), 'SynF':np.zeros(10,dtype=float), 
          'Model Zoo':np.zeros(10,dtype=float), 'Prog-NN':np.zeros(10,dtype=float), 
          'LMC':np.zeros(10,dtype=float), 'DF-CNN':np.zeros(10,dtype=float),  
          'SynF (constrained)':np.zeros(10,dtype=float), 'EWC':np.zeros(10,dtype=float),
...
for count,name in enumerate(te_5000.keys()):
    for i in range(10):
        if count <6:
            ...
            te_5000[name][i] = np.log(tes_bottom[count-6][i][9-i])


df_5000 = pd.DataFrame.from_dict(te_5000)
df_5000 = pd.melt(df_5000,var_name='Algorithms', value_name='Learning Efficieny')

for name in te_5000.keys():
    print(name, np.round(np.mean(te_5000[name]),2), np.round(np.std(te_5000[name], ddof=1),2))

# %%
fig = plt.figure(constrained_layout=True,figsize=(42,32))
gs = fig.add_gridspec(32,42)

clr_top = ["#377eb8", "#e41a1c", "#4daf4a", "#984ea3", "#984ea3", "#f781bf"]
...
marker_style = ['.', '.', '.', '.', 'o', '+', '.', '+',  'v', '.', 'o', '*', '.', '+', 'x', 'o']
marker_style_scatter = ['.', '.', '.', '.', 'o', '+', 'v', '.', '+', '.', '.', 'o', '*', '.', '+', 'x', 'o']

clr_combined = ["#377eb8", "#e41a1c", "#4daf4a", "#984ea3", "#984ea3", "#f781bf", "#f781bf", "#b15928", "#b15928", "#984ea3", "#f781bf", "#f781bf", "#b15928", "#b15928", "#b15928", "#b15928"]
c_combined = sns.color_palette(clr_combined, n_colors=total_alg_top+total_alg_bottom-1)

clr_combined_ = ["#377eb8", "#e41a1c", "#4daf4a", "#984ea3", "#984ea3", "#f781bf", "#e41a1c", "#b15928", "#b15928", "#984ea3", "#f781bf", "#f781bf", "#f781bf", "#b15928", "#b15928", "#b15928", "#b15928"]
...
ax_.set_xticklabels(
    ['SynN','SynF', 'Model Zoo','ProgNN', 'LMC', 'DF-CNN', 'SynF (constrained)','EWC', 'Total Replay', 'Partial Replay', 'LwF', 'O-EWC','SI', 'ER', 'A-GEM', 'TAG', 'None'],
