...
#%%
def LF_experiment(train_x, train_y, test_x, test_y, noise_size, ntrees, shift, slot, model, num_points_per_task, acorn=None):
    ...
    file_to_save = 'result/'+model+str(num_points_per_task)+'_'+str(ntrees)+'_'+str(shift)+'_'+str(slot)+'.pickle'
...
#%%
def run_parallel_exp(data_x, data_y, noise_size, n_trees, model, num_points_per_task, slot=0, shift=1):
    ...
    if model == "dnn":
        ...
        LF_experiment(train_x, train_y, test_x, test_y, noise_size, n_trees, shift, slot, model, num_points_per_task, acorn=12345)
...
if model == "uf":
    ...
    Parallel(n_jobs=-2,verbose=1)(
        delayed(run_parallel_exp)(
                data_x, data_y, noise_size, ntree, model, num_points_per_task, slot=slot, shift=shift
